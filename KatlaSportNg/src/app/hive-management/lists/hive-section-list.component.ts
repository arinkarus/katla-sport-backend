...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number) {
    var requiredHiveSection = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveService.setHiveStatus(hiveSectionId, true).subscribe(c => requiredHiveSection.isDeleted = true);
  }

  onUndelete(hiveSectionId: number) {
    var requiredHiveSection = this.hiveSections.find(h => h.id == hiveSectionId);
    this.hiveService.setHiveStatus(hiveSectionId, false).subscribe(c => requiredHiveSection.isDeleted = false);
  }
}
